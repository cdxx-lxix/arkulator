    ...
    <ArkFrame legend="budget.header">
        ...
        <div class="flex flex-col gap-2 w-full px-2 sm:px-6 justify-start font-light text-gray-100 text-md">
            <div class="flex flex-row flex-wrap gap-3 w-full justify-start text-lg font-bold">
                <p class="flex flex-row gap-2 items-center justify-center" :class="totalPulls > 0 ? 'text-blue-ribbon-700' : totalPulls === 0 ? 'text-amber-300' : 'text-red-600'">
                    <img :src="recruit_bag" alt="recruitment bag" class="w-6 h-6"> {{ totalPulls }}
                </p>
            </div>
            <hr>
            <div v-for="entry, index in budget_entries" :key="entry">
                <div v-if="budget_entries[index].operator"
            ...
            <hr>
            <div v-for="entry, index in budget_entries" :key="entry" class="w-full grid grid-cols-10 gap-3">
                <div class="col-span-3">
                    <label class="block mb-2 text-sm font-medium text-white">{{ $t('budget.operator_field') }}</label>
                    <input type="text" v-model="budget_entries[index].operator"
                ...
                <button class="col-span-1 flex justify-center flex-col items-center h-full cursor-default">
                    ...
                    <RemoveButtonIcon @click="removeEntry(index)"
                        class="w-6 h-6 fill-blue-ribbon-700 hover:fill-red-600 ease-in-out delay-75 transition-opacity cursor-pointer" />
                </button>
            </div>
            <button
                class="flex justify-center items-center py-2 mt-2 rounded-lg hover:bg-blue-ribbon-700 fill-blue-ribbon-700 hover:fill-white"
                @click="addNewEntry">
...
const orundum_local = ref(props.orundum)
const permits_local = ref(props.permits)
const banner_types = [
